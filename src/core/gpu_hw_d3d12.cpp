...
#define UPDATE_PROGRESS()                                                                                              \
  ...
  ComPtr<ID3DBlob> fullscreen_quad_vertex_shader = shader_cache.GetVertexShader(shadergen.GenerateScreenQuadVertexShader());
  ...
  // VRAM fill
  {
    ...
    {
      ComPtr<ID3DBlob> fs = shader_cache.GetPixelShader((interlaced == 0) ? shadergen.GenerateFillFragmentShader() :
  ...
  // Display
  {
    ...
    for (u8 depth_24 = 0; depth_24 < 2; depth_24++)
    {
      for (u8 interlace_mode = 0; interlace_mode < 3; interlace_mode++)
      {
        ...
        m_display_pipelines[depth_24][interlace_mode] = gpbuilder.Create(g_d3d12_context->GetDevice(), shader_cache, false);
